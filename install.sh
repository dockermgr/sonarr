...
# @@File             :  install.sh
# @@Description      :  Container installer script for sonarr
...
REPO="${DOCKERMGRREPO:-https://github.com/$SCRIPTS_PREFIX}/sonarr"
APPVERSION="$(__appversion "$REPO/raw/$REPO_BRANCH/version.txt")"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Defaults variables
APPNAME="sonarr"
export INSTDIR="$HOME/.local/share/CasjaysDev/$SCRIPTS_PREFIX/sonarr"
export DOCKERMGR_CONFIG_DIR="${DOCKERMGR_CONFIG_DIR:-$HOME/.config/myscripts/$SCRIPTS_PREFIX}"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set the mountpoint directory
export APPDIR="$HOME/.local/share/srv/docker/sonarr"
...
HUB_IMAGE_URL="casjaysdevdocker/sonarr"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set container hostname and domain - Default: [sonarr.$SET_HOST_FULL_NAME] [$SET_HOST_FULL_DOMAIN]
CONTAINER_HOSTNAME=""
...
CONTAINER_ADD_CUSTOM_PORT="0.0.0.0:8989:8989"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
CONTAINER_MOUNTS="/mnt/tv:/tv,/mnt/downloads:/downloads"
CONTAINER_MOUNTS+=""
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
CONTAINER_HOSTNAME="${CONTAINER_HOSTNAME:-${APPNAME:-sonarr}}"
...
if [ -n "$SONARR_USERNAME" ]; then
  CONTAINER_USER_NAME="$SONARR_USERNAME"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
  CONTAINER_USER_NAME="${SONARR_USERNAME:-${CONTAINER_USER_NAME:-$DEFAULT_USERNAME}}"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
...
if [ -n "$SONARR_PASSWORD" ]; then
  CONTAINER_USER_PASS="$SONARR_PASSWORD"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
  if [ "$CONTAINER_USER_PASS" = "random" ]; then
    CONTAINER_USER_PASS="$(__password "${CONTAINER_PASS_LENGTH:-16}")"
  fi
  CONTAINER_USER_PASS="${SONARR_PASSWORD:-${CONTAINER_USER_PASS:-$DEFAULT_PASSWORD}}"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
